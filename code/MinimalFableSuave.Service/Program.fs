...
let main argv =
    ...
    let start hc = 
        ...
        let config = MinimalFableSuave.Routing.config cts.Token ipAddress port
        printfn "IPAddres received from config file %s" ipAddress
        printfn "Port received from config file %i" port

        startWebServerAsync config MinimalFableSuave.Routing.app
        |> snd
        |> Async.StartAsTask 
        |> ignore

        cancellationTokenSource := Some cts
        MinimalFableSuave.Tasks.createTasks()
        true
    ...
    let stop hc = 
        ...
        | Some cts -> cts.Cancel()
        | None -> ()
        MinimalFableSuave.Tasks.stopTasks()
        true
